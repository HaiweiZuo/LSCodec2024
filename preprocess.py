...
def load_data(dataset):
    ...
    if dataset == 'synthetic':
        ...
        dataset_folder = 'data/KPI'
        train_file = os.path.join(dataset_folder, "phase2_train.csv")
        test_file = os.path.join(dataset_folder, "phase2_ground_truth.hdf")
        data1, labels1, timestamps1, data2, labels2, timestamps2 = _load_raw_KPI(train_file, test_file)
        ...
        for ky in all_train_data.keys():
            if ky not in all_train_labels.keys():
                continue
            train_data = all_train_data[ky]
            label_data = all_train_labels[ky]
            np.save(os.path.join(folder, "{}_train.npy".format(ky[:13])), train_data)
            np.save(os.path.join(folder, "{}_labels.npy".format(ky[:13])), label_data)
        for ky in all_test_data.keys():
            if ky not in all_test_labels.keys():
                continue
            test_data = all_test_data[ky]
            label_data = all_test_labels[ky]
            np.save(os.path.join(folder, "{}_test.npy".format(ky[:13])), test_data)
            np.save(os.path.join(folder, "{}_labels.npy".format(ky[:13])), label_data)
    else:
...
if __name__ == '__main__':
    commands = []  # ['MBA', 'SMAP', 'MSL', 'SMD', 'synthetic', 'KPI'] # sys.argv[1:]
    load = []
    if len(commands) > 0:
        for d in commands:
            load_data(d)
